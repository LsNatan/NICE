...
class ResNeXt_imagenet(ResNeXt):
    ...
    def __init__(self, num_classes=1000,
                 block=Bottleneck, layers=[3, 4, 23, 3], batch_norm=True, shortcut='B'):
...
def resnext_original(**kwargs):
    num_classes, depth, dataset, batch_norm, shortcut = map(
        kwargs.get, ['num_classes', 'depth', 'dataset', 'batch_norm', 'shortcut'])
    ...
    if dataset == 'imagenet':
        ...
        if depth == 18:
            return ResNeXt_imagenet(num_classes=num_classes,
                                   ...
                                   batch_norm=batch_norm, shortcut=shortcut)
        if depth == 34:
            return ResNeXt_imagenet(num_classes=num_classes,
