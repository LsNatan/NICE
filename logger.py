...
class CsvLogger:
    ...
    def write(self, data, file_name=None):
        path = self.csv_path if file_name == None else os.path.join(self.log_path, file_name)
        for k in self.data:
    ...
    def write_text(self, text, print_t=True, file_name='params.txt'):
        with open(os.path.join(self.log_path, file_name), 'a') as f:
            f.write('{}\n'.format(text))
        if print_t:
            print(text)
